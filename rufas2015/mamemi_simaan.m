...
A = @(E_t, D_m, D_a) [-(D_m/R_m + D_a/R_a)*E_t, D_m/R_m, 0, D_a/R_a, 0 ;
    (D_m * E_t)/(R_m * C_ae), -(C_ae^-1)*(1/R_s + D_m/R_m), 0, 0, (R_s*C_ae)^-1;
    0, 0, -R_c/L_s, L_s^-1, -L_s^-1;
    (D_a * E_t)/(R_a*C_ao), 0, -C_ao^-1, -D_a/(R_a*C_ao), 0;
...
while t < t_end
    ...
    % RK4
    dx = A(data.E(n), D_m, D_a) * x +  p(data.E(n), D_m, D_a);
    kx1 = T_s * dx;
    x1 = x + 0.5*kx1;
    
    dx = A(data.E(n), D_m, D_a) * x1 +  p(data.E(n), D_m, D_a);
    kx2 = T_s * dx;
    x1 = x + 0.5*kx2;
    ...
    data.P_ve(n+1) = P_ve(data.E(n+1), data.x.V_ve(n+1));
...
figure(1);
subplot(3,1,1);
plot(data.t, data.x.P_ao(1:end-1), 'k', data.t, data.P_ve(1:end-1), ':k', data.t, data.x.P_ae(1:end-1), '--k');
...
subplot(3,1,2);
plot(data.t, data.x.Q_a(1:end-1), 'k');
ylabel('Aortic Flow (ml/s)');
axis([0, 8, -50, 700]);
grid on;

subplot(3,1,3);
plot(data.t, data.x.V_ve(1:end-1), 'k');
ylabel('LVV (ml)');
xlabel('time (s)');
axis([0, 8, 50, 160]);
grid on;
...
subplot(2,1,2);
plot(data.t(1:end-1),data.E(1:end-2),'b', data.t(1:end-1),40*max(data.x.zecg)*R_dtct,'k');
title('Elastance');
legend('E(t)', 'MaMeMi');
axis([0, 8, -0.2, 2.2]);
grid on;